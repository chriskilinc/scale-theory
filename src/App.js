...
function App() {
  ...
  const onScalesClick = (e) => {
    setScalesVisible(!scalesVisible);
              ...
              <section className={`piano ${pianoVisible ? 'visible' : 'hidden'}`}>
                <div className="piano-container">
                  {piano.map((note, i, arr) => {
                    ...
                    const active = scale.notes.includes(note);
                    const isBlack = note.includes("#");
                    const pianoKey = isBlack ? "black" : "white";
                    const isKey = (note == scale.notes[0]);
                ...
                <div className="guitar-container">
                  {
                    guitar.strings.map((string, i, arr) => {
                      ...
                      const notes = getArrangedChromaticScaleByKey(string);
                      notes.push(notes[0]);

                      return <div id={id} key={id} string={string} className={classes}>
                        {
                          [...Array(guitar.frets + 1)].map((fret, i) => {
                            const note = notes[i];
                            const active = scale.notes.includes(note);
                            const isKey = (note == scale.notes[0]);
